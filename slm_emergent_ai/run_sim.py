...
from .eval.metrics import MetricsServer
...
async def run_simulation(config: Dict[str, Any]):
    ...
    for key, value in lambda_params.items():
        bb.set_param(key, value)
    
    # 基準速度を設定
    bb.set_param("base_speed", 10.0)
    
    print(f"Initializing {agent_count} agents...")
    for i in range(agent_count):
        role = f"Agent{i+1}"
        agent = SLMAgent(id=i+1, role=role, model=model, λ=lambda_params)
        agents.append(agent)
    ...
    # メトリクスサーバーの初期化
    metrics_port = config.get("metrics", {}).get("port", 7861)
